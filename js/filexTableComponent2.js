...
var FlexTables = (function (exports) {
  ...
  function initReconstructData(table, elthead, eltheadCells, fixedSells) {
  ...
  function removeFixedRows( parentBox) {
    parentBox.removeChild(parentBox.childNodes[2]);
   }
  function setRightFixedRows(createParentBox) {
     ...
     fixedRowsPro.setAttribute('style', "position: relative;  overflow: hidden");
     var fixedRowsHead = document.createElement("div");
     fixedRowsHead.setAttribute("class", 'fixed-rightRows-head');
     fixedRowsHead.setAttribute('style', "position: absolute; left:0, top:0;");
     var fixedRowBody = document.createElement("div");
     fixedRowBody.setAttribute('class', "fixed-rightRrows-body scrollbar-no");
     fixedRowBody.setAttribute('style', "overflow-y: auto;overflow-x: hidden;");
     fixedRowsPro.appendChild(fixedRowBody);
     fixedRowsPro.appendChild(fixedRowsHead);
     fixedRowsBox.appendChild(fixedRowsPro);
  ...
  function filexTble(el, height, scrollbar) {
    ...
    initReconstructData(el,elthead,eltheadCells,fixedSells);
  ...
  function reconstructData(insertIndexs, Eltbody) {
...
/*
    var cellIndexs = [];
    _.forEach(eltheadCells,function (cell) {
      if (cell.getAttribute('index') != null) {
        cellIndexs.push(cell.getAttribute('index'));
      }
    });
    reconstructData(cellIndexs,elthead);
    reconstructData(cellIndexs, Eltbody);*/
    ...
    var isFixedRows = false;

    if (fixedSells.length > 0 && !fixedRowsHead && !fixedRowsBody) {
      setFixedRows(grandpa);
      isFixedRows = true;
    }
    if (fixedSells.length == 0 &&  fixedRowsHead &&  fixedRowsBody) {
      removeFixedRows(grandpa);
      isFixedRows = true;
    }
     fixedHeadBox = grandpa.getElementsByClassName('fixed-head-box')[0];
     fixedRowsHead = grandpa.getElementsByClassName('fixed-rows-head')[0];
     fixedRowsBody = grandpa.getElementsByClassName('fixed-rows-body')[0];
    ...
    if (fixedRowsBody && fixedRowsBody.innerHTML) fixedRowsBody.innerHTML = null;
    if (fixedRowsHead && fixedRowsHead.innerHTML) fixedRowsHead.innerHTML = null;
    if (fixedHeadBox && fixedHeadBox.innerHTML) fixedHeadBox.innerHTML = null;
